...
class SwitchMode(Node):
    ...
    def __init__(self):
        ...
        description_pkg = get_package_share_directory('switch_mode_launch')
    ...
    def mode_callback(self, msg):
        ...
        if new_mode == 'navi':
            ...
            #  ros2 launch ${ROM_ROBOT_MODEL}_carto mapping.launch.py use_rviz:=False use_sime_time:=False
            
            self.current_process = subprocess.Popen(['ros2', 'launch', 'rom2109_carto', 'navigation.launch.py'])
            self.current_mode = new_mode

        elif new_mode == 'mapping':
            self.get_logger().info('Switching to mapping mode...')
            self.current_process = subprocess.Popen(['ros2', 'launch', 'rom2109_carto', 'mapping.launch.py'])
            self.current_mode = new_mode

        elif new_mode == 'remapping':
            self.get_logger().info('Switching to remapping mode...')
            #self.current_process = subprocess.Popen(['ros2', 'launch', 'rom2109_carto', 'remapping.launch.py'])
            self.current_mode = new_mode
